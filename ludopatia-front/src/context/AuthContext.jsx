...
const AuthContextProvider = ({ children }) => {
    ...
    const [sessionExpDate, storeSessionExpDate, clearStoredSessionExpDate] = useLocalStorage('sessionExpDate');
    ...
    useEffect(() => {
        if (currentUser && sessionExpDate) {
            const remaininTime = new Date(sessionExpDate).getTime() - new Date().getTime();
            logoutTimer = setTimeout(handleAutomaticLogout, remaininTime);
        } else {
